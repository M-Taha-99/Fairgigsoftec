    ...
    <div className="animate-fade-in" style={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column' }}>
      <header className="dashboard-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <h1 className="header-title">Audit Queue</h1>
            <p className="header-subtitle">Reviewing {queue.length} pending shifts for digital integrity.</p>
          </div>
        </div>
      </header>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1.2fr 2fr', gap: '1.5rem', overflow: 'hidden' }}>
        ...
        <div className="chart-box" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '1.5rem', borderBottom: '1px solid #eee' }}>
                <h3 className="chart-title">Pending Review</h3>
            </div>
                ...
                {queue.map(item => (
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.75rem' }}>{item.platform} • {item.hours_worked} hrs</span>
            ...
            {selectedItem ? (
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <div style={{ padding: '1.5rem', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <h3 className="chart-title">Inspection: {workerMap[selectedItem.worker_id] || selectedItem.worker_id}</h3>
                            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Shift Date: {selectedItem.shift_date || 'N/A'}</p>
                        </div>
                        <button onClick={runAnomalyAssist} className="btn-download" style={{ background: 'var(--accent-teal)', padding: '0.4rem 0.8rem', fontSize: '0.75rem' }}>
                            <Search size={14} /> AI Anomaly Assist
                        </button>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <h4 style={{ fontSize: '0.8rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Digital Evidence</h4>
                            <div style={{ flex: 1, minHeight: '300px', background: '#f1f5f9', borderRadius: '12px', overflow: 'hidden', position: 'relative', border: '1px solid #e2e8f0' }}>
                                {selectedItem.evidence_url ? (
                                    ...
                                    <div style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'var(--text-muted)' }}>No digital evidence attached</div>
                                )}
                            </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                <h4 style={{ fontSize: '0.8rem', textTransform: 'uppercase', color: 'var(--accent-blue)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <FileText size={16} /> Audit Comparison
                                </h4>
                                
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    <div style={{ borderRight: '1px solid #e2e8f0', paddingRight: '1rem' }}>
                                        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>WORKER REPORTED</p>
                                    ...
                                    <div>
                                        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>VERIFIER OBSERVED</p>
                                        <p style={{ fontSize: '0.65rem', color: '#64748b', fontStyle: 'italic' }}>Check screenshot and confirm if figures match.</p>
                                        <div style={{ marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem' }}>
                                                <input type="checkbox" id="gross_match" /> <label htmlFor="gross_match">Gross Matches</label>
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem' }}>
                                                <input type="checkbox" id="fee_match" /> <label htmlFor="fee_match">Fee Matches</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div style={{ marginTop: 'auto' }}>
                                <label style={{ fontSize: '0.8rem', fontWeight: 'bold' }}>Verifier Note</label>
                                <textarea 
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: 'var(--text-muted)' }}>
                    ...
                    <p>Select a shift from the queue to begin inspection.</p>
                </div>
            )}
        </div>
